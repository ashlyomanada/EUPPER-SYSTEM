<template>
  ...
  <div
    class="modal fade"
    id="occidentalExcelConfirmation"
    tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-body text-center">
          <p>
            Are you sure do you want to generate an Excel Report for Occidental
            Mindoro?, please check if all users rated!
          </p>
  ...
  >
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-body text-center">
          <p>
            Are you sure do you want to generate an PDF Report for Occidental
            Mindoro?, please check if all users rated!
          </p>
  ...
  methods: {
    confirmationGenerateExcel() {
      const modalElement = document.getElementById(
        "occidentalExcelConfirmation"
      );
      const modalInstance = new Modal(modalElement);
      modalInstance.show();
    },
    confirmationGeneratePdf() {
      const modalElement = document.getElementById("occidentalPdfConfirmation");
      const modalInstance = new Modal(modalElement);
      modalInstance.show();
    },
    async fetchColumns() {
    ...
    async getUsersRate() {
      try {
        ...
        const response = await axios.post("/getUsersRateOcci", {
          Month: currentMonth,
          Year: currentYear,
        });
    ...
    async getUsersRateByMonth() {
      try {
        const response = await axios.post("/getUsersRateOcciByMonth", {
          Month: this.month,
          Year: this.year,
        });
    ...
    async generateOcciReport() {
      try {
        const response = await axios.post(
          `${this.baseURL}generateOcciReport`,
          {
            month: this.month,
            year: this.year,
          },
    ...
    generatePdf() {
      ...
      axios
        ...
        .then((response) => {
          ...
          a.href = url;
          a.download = `Occidental_Mindoro_Report_${this.month}_${this.year}.pdf`;
          document.body.appendChild(a);
          a.click();
          window.URL.revokeObjectURL(url);

          const modalElement = document.getElementById(
            "occidentalPdfConfirmation"
          );
          const modalInstance =
            Modal.getInstance(modalElement) || new Modal(modalElement);
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 404) {
            this.errorMessage =
              "Users Rate must be completed before generating PDF";
          } else if (error.response && error.response.status === 500) {
            this.errorMessage = "Server error. Please try again later.";
          } else {
            this.errorMessage =
              "Failed to generate PDF Report, Please Check your connection.";
          }
        });
    },
  },
