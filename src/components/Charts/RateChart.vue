  ...
  <div class="adminBox shadow">
    ...
    >
      ...
      <div class="d-flex gap-2">
        ...
        >
          <option
            v-for="userinfos in usersInfo"
            :key="userinfos.user_id"
            :value="userinfos.user_id"
          >
            {{ userinfos.office }}
          </option>
        </select>
...
export default {
  ...
  async mounted() {
    ...
    const selectElement = document.querySelector("#selectedTable4");
  ...
  methods: {
    async getUsersInfo() {
      try {
        const response = await axios.get("getUsersInfo");
      ...
      this.chartInstance = new Chart(context, {
        ...
        options: {
          ...
          plugins: {
            title: {
              display: true,
              text: `All rating of ${this.selectedText}`,
            },
          },
        },
      });
    ...
    getRandomColor() {
      const r = Math.floor(Math.random() * 156) + 50; // Random value between 50 and 205
      const g = Math.floor(Math.random() * 156) + 50; // Random value between 50 and 205
      const b = Math.floor(Math.random() * 156) + 50; // Random value between 50 and 205

      // Darker background with lighter opacity
      return `rgba(${r}, ${g}, ${b}, 0.8)`; // Opacity closer to 1 makes the color darker
    },
  },
};
</script>
