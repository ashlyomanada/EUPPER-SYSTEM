    ...
    <div class="order">
      <div class="rating-header">
        <div>
          ...
          <h4 class="head-subtitle">New Office for Palawan</h4>
        </div>
      </div>
      <form class="form" @submit.prevent="addColumnPal">
        <input
          type="text"
          ...
          required
        />
        <button type="submit">Add Palawan Office</button>
      </form>
    </div>
  </div>
  ...
  methods: {
    async addColumnPal() {
      try {
        ...
        const ins = await axios.post("/addColumnPal", {
