    ...
    <div class="order">
      <div class="rating-header">
        <div>
          ...
          <h4 class="head-subtitle">New Office for Occidental Mindoro</h4>
        </div>
      </div>
      <form class="form" @submit.prevent="addColumnOcci">
        <input
          type="text"
          ...
          required
        />
        <button type="submit">Add Occidental Mindoro Office</button>
      </form>
    </div>
  </div>
  ...
  methods: {
    async addColumnOcci() {
      try {
        ...
        const ins = await axios.post("/addColumnOcci", {
