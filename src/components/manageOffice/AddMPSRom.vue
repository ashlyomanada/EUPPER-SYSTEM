    ...
    <div class="order">
      <div class="rating-header">
        <div>
          ...
          <h4 class="head-subtitle">New Office for Romblon</h4>
        </div>
      </div>
      <form class="form" @submit.prevent="addColumnRom">
        <input
          type="text"
          ...
          required
        />
        <button type="submit">Add Romblon Office</button>
      </form>
    </div>
  </div>
  ...
  methods: {
    async addColumnRom() {
      try {
        ...
        const ins = await axios.post("/addColumnRom", {
