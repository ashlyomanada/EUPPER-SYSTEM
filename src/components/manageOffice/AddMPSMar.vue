    ...
    <div class="order">
      <div class="rating-header">
        <div>
          ...
          <h4 class="head-subtitle">New Office for Marinduque</h4>
        </div>
      </div>
      <form class="form" @submit.prevent="addColumnMar">
        <input
          type="text"
          ...
          required
        />
        <button type="submit">Add Marinduque Office</button>
      </form>
    </div>
  </div>
  ...
  methods: {
    async addColumnMar() {
      try {
        ...
        const ins = await axios.post("/addColumnMar", {
